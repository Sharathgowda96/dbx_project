...
```bash
conda create -n cicd_sample python=3.9
conda activate cicd_sample
```
...
```
This command will create your personal repository under `/Repos/<username>/cicd_sample`.
...
- To trigger the release pipeline, get the current version from the `cicd_sample/__init__.py` file and tag the current code version:
```
